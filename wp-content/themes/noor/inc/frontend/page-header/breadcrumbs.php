<?php
// Breadcrumbs
if ( ! function_exists( 'noor_get_breadcrumbs' ) ) {
    function noor_get_breadcrumbs ( 
        $list_style = 'ul', 
        $list_id = 'breadcrumbs', 
        ...
        // Front page
        if ( is_front_page() ) {
            $breadcrumb .= '<li class="' . $active_class . '">' . esc_html__('Home', 'noor') . '</li>'; //get_bloginfo( 'name' )
        } else {
            $breadcrumb .= '<li><a href="' . home_url() . '">' . esc_html__('Home', 'noor') . '</a></li>'; //get_bloginfo( 'name' )
        }
        // Blog archive
        ...
        // Search
        if ( is_search() ) {
            $breadcrumb .= '<li class="' . $active_class . '">' . esc_html__( 'Search', 'noor' ) . '</li>';
        }
        // 404
        if ( is_404() ) {
            $breadcrumb .= '<li class="' . $active_class . '">' . esc_html__( '404', 'noor' ) . '</li>';
        }
        ...
        if ( is_single() && get_post_type() != 'post' && get_post_type() != 'attachment' ) {
            ...
            if ( get_post_type_archive_link( $cpt_obj->name ) ) {
                ...
                $archive_link = '';
                if ( 'noor_portfolio' == get_post_type() && noor_get_option('portfolio_archive') == 'archive_custom' ) {
                    $archive_link = get_page_link( noor_get_option('archive_page_custom') );                                
                } else {
...
if ( ! function_exists( 'noor_breadcrumbs' ) ) {
    function noor_breadcrumbs (){

        if( noor_get_option('breadcrumbs') && !is_front_page() ){ echo noor_get_breadcrumbs(); } ?>
